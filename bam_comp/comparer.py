...
#   ----------------------------------------------------------------------------------
#   Total_FASTQ_reads   | Initial number of reads           |      Always 100%       |
#                       | within FASTQ File                 |                        |
#   ----------------------------------------------------------------------------------
#   Total_BAM_reads     | Number of alignments              | from Total_FASTQ_reads |
#                       | within BAM File                   |                        |
#   ----------------------------------------------------------------------------------
#   Mapped_reads        | Number of reads without those     |                        |
#                       | which were mapped neither with    | from Total_FASTQ_reads |
#                       | original nor with replicated data |                        |
#   ----------------------------------------------------------------------------------
#   Mapped_reads_type1  | Number of reads mapped with       |                        |
#                       | original data                     | from Total_FASTQ_reads |
#                       |                                   |                        |
#   ----------------------------------------------------------------------------------
#   Mapped_reads_type2  | Number of reads mapped with       |                        |
#                       | replicated data                   | from Total_FASTQ_reads |
#                       |                                   |                        |
#   ----------------------------------------------------------------------------------
#   Unambiguous_type1   | Number of unambiguous reads       |   from Mapped_reads    |
#                       | from original data                |                        |
#   ----------------------------------------------------------------------------------
#   Unambiguous_type2   | Number of unambiguous reads       |   from Mapped_reads    |
...
class Comparer:
    ...
    def extract_mapped(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if sum(self.comparable) == 2:
            ...
            df1 = self.imported_dfs[0]
            df2 = self.imported_dfs[1]
            ...
            self.imported_dfs = [df_mapped_g, df_mapped_rep]
            df_mapped = df.loc[
    ...
    def extract_unambiguous(self, df_m: pd.DataFrame, df_c: pd.DataFrame) -> pd.DataFrame:
        ...
        if sum(self.comparable) == 2:
            l1, l2 = self.get_labels()
            df_unambiguous = df_m.loc[
                df_m[f'{l1}_multi'].isin(val) &
                df_m[f'{l2}_multi'].isin(val)
                ]

            df_common_unambiguous = df_c.loc[
                df_c[f'{l1}_multi'].isin(val) &
                df_c[f'{l2}_multi'].isin(val)
                ]
    ...
    def compare(self):
        ...
        #    nor replicated data
        df_mapped, df_common = self.extract_mapped(df)
